...
class PocketBaseClient:
    ...
    def update_user(self, user_id, **fields):
        ...
        url = f"{self.url}/api/collections/users/records/{user_id}"
        response = requests.patch(url, json=fields, headers=self._headers())
        if response.status_code != 200:
            raise Exception(f"Error al actualizar usuario: {response.text}")
        return response.json()
    ...
    def get_or_create_user(self, telegram_id, telegram_username, display_name):
        ...
        NO sobrescribe display_name si el usuario ya existe.
        """
        user = self.get_user_by_telegram_id(telegram_id)
        if user:
            # Solo actualizar telegram_username si cambió (info de Telegram)
            cambios = {}
            if (user.get("telegram_username") or "") != (telegram_username or ""):
                cambios["telegram_username"] = telegram_username or ""

            if cambios:
                user = self.update_user(user["id"], **cambios)
